...
class Trainer(metaclass=ABCMeta):
    ...
    def train_bert_one_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(tqdm_dataloader):
            ...
            self.optimizer.zero_grad()
            logits, y = self.model(seqs)
            # y = y.detach().cpu().numpy() if y.requires_grad else y.cpu().numpy()
            y1 = y
            y = torch.round(y)
            # y = build_overall_sequence(y)
            # logits = build_overall_sequence(logits_on)
            labels = labels_energy / self.cutoff
            logits_energy = self.cutoff_energy((logits * self.cutoff * y))
            logits_status = self.compute_status(logits_energy)
            ...
            for i in range(len(logits_masked)):
                if labels_masked[i] <= 50 / self.cutoff:  # 60/self.cutoff:
                    ...
                    square = subtract * subtract
                    harm += square
            mse_loss1 = harm / len(logits_masked)
            ...
            # bce_loss = nn.BCEWithLogitsLoss(torch.log(F.softmax(logits_masked.squeeze() / 0.1, dim=-1) + 1e-9), F.softmax(labels_masked.squeeze() / 0.1, dim=-1))
            total_loss = kl_loss + mse_loss + margin_loss + bce_loss
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm_dataloader):
                ...
                # y = y.detach().cpu().numpy() if y.requires_grad else y.cpu().numpy()
                # y = build_overall_sequence(y)
                # logits = build_overall_sequence(logits)
                labels = labels_energy / self.cutoff
                logits_energy = self.cutoff_energy((logits * self.cutoff * y))
                logits_status = self.compute_status(logits_energy)
    ...
    def test(self, test_loader):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm_dataloader):
                ...
                labels = labels_energy / self.cutoff
                logits_energy = self.cutoff_energy((logits * self.cutoff * y))
                # logits_energy = self.cutoff_energy((logits * self.x_std)+self.x_mean)
                logits_status = self.compute_status(logits_energy)
                logits_energy = logits_energy * torch.round(y)
        ...
        # ax.plot(e_pred_curve, color='green', label='pred')
        ax.plot(ground_truth, color='yellow', label='main power')
        plt.xticks(fontsize=25)
        plt.yticks(fontsize=25)
        plt.legend()
        plt.show()
        fig.savefig('3.svg', format='svg')  # 输出
        # g = make_dot(logits)
        # g.view()
